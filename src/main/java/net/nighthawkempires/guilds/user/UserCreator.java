...
public class UserCreator {
    ...
    public void create() {
        if (NECore.getSettings().useSQL) {
            try {
                PreparedStatement statement = NECore.getMySQL().getConnection().prepareStatement("SELECT * FROM guilds_data WHERE uuid=?");
                statement.setString(1, getUser().getUUID().toString());
                ResultSet results = statement.executeQuery();
                results.next();
                if (!NEGuilds.getUserManager().userExists(getUser().getUUID())) {
                    PreparedStatement insert = NECore.getMySQL().getConnection().prepareStatement("INSERT INTO guilds_data(" +
                    ...
                    NECore.getLoggers().info("Created Guilds User " + getUser().getUUID().toString() + ": " + Bukkit.getOfflinePlayer(getUser().getUUID()).getName() + ".");
                }
